...
| `context-diagram.png`  | `mova context-trace ` | Evidencia **visual**: fuentes → gobernanza (Sanitizer/PII/Circuit Breaker) → agente → tokens → costo, en un solo vistazo. | Sí — evidencia primaria |
| `pii-compliance-governance.png`  | `mova run 02-pii-compliance-governance --diagram --export png --path ./docs/assets/pii-compliance-governance.png` | Evidencia **visual**: Diagrama de arquitectura/gobernanza exportado a una ruta personalizada para documentación o activos de repositorio (./docs/assets/). | Sí — evidencia primaria |
