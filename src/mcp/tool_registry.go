...
func tools() []map[string]any {
	return []map[string]any{
		...
		tool("chat_completion", "Send a message to a local model (Ollama, LM Studio, vLLM...) configured under config/models/. Optionally attaches the full Mova context (project+task) as the system prompt. Natural language that asks to MODIFY an existing file (\"fix the bug in auth.go\", \"update report.md\") is detected automatically: the proposed change (a precise line diff) is returned but NOT written unless `apply_edits` is true — there's no interactive y/n on this door, so pass `apply_edits: true` on the same message once you've reviewed the diff to actually write it. If the project's project.json declares \"egress_audit\" (see PROJECT_JSON.md § egress_audit), the sanitized context is logged and/or the call may stop as a dry run BEFORE any provider is reached — a dry run still returns a normal, successful reply saying no inference happened.",
