...
func runMemoryClear(adapter Adapter, root, project string) {
	...
	needConfirm := true
	if proj != nil {
		needConfirm = confirmDeleteRequired(proj.Archive)
	}
...
func clearDescription(req MemoryDeleteRequest, project string) string {
	switch {
	case req.All:
